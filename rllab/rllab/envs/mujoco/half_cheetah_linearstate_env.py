...
class HalfCheetahLinearStateConfig(TaskConfig):
    """
    ...
    def __init__(self, n_params=18, load_state=True):
        ...
        if load_state == True:
            ...
            #        assert(False)
            lo, hi = np.array([-1.0]*n_params), np.array([1.0]*n_params)
        else:
            lo, hi = np.array([-5.0]*n_params), np.array([5.0]*n_params)
        self.coef = np.array([1.0] * n_params)
        print (f'coef = {self.coef}')

        TaskConfig.__init__(self, n_params, lo, hi, scale_psi=False)
...
class HalfCheetahLinearStateEnv(HalfCheetahEnv):
    """
        A half cheetah environment with a configurable goal velocity.
    """
    _task_config: HalfCheetahLinearStateConfig
    ...
    def __init__(self, task={}, n_tasks=100, randomize_tasks=False, *args, **kwargs):
        ...
        super(HalfCheetahLinearStateEnv, self).__init__(*args, **kwargs)
    ...
    def step(self, action):
        ...
        reward_run = np.sum(norm_next_obs * self._task_config.goal_velocity * self._task_config.coef)
